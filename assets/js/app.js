...
const nokCarreta = {};
const nokFotos   = {}; // { numeroDoItem: [ dataURL(JPEG comprimido), ... ] }
const FOTO_MAX   = 6;  // limite de fotos por item (evita PDF gigante)
      ...
      if (semChk) {
        ...
        tr.innerHTML = `
          ...
          <td><div class="chk-grupo">
            ...
            <button type="button" class="foto-badge" id="i${n}_fotobadge" style="display:none" onclick="reabrirFoto(${n})" title="Ver / refazer foto">📷</button>
          </div></td>`;
...
/* ══════════════════════════════════════════════════════
   POPUP FOTO (item NOK) — suporta múltiplas fotos por item
   ══════════════════════════════════════════════════════ */
let _fotoItemAtual = null;
let _fotosAtual    = []; // cópia de trabalho, só vai pra nokFotos[n] ao "Concluir"

function pedirFoto(n, descricao) {
  _fotoItemAtual = n;
  _fotosAtual = (nokFotos[n] || []).slice();

  document.getElementById('foto-desc').textContent = `${n}. ${descricao}`;
  document.getElementById('foto-input').value = '';

  renderGaleriaFoto();
  document.getElementById('popup-foto').classList.add('aberto');
}

function reabrirFoto(n) {
  pedirFoto(n, desc(itemDe(n)));
}

function renderGaleriaFoto() {
  const galeria = document.getElementById('foto-galeria');
  const wrap    = document.getElementById('foto-galeria-wrap');
  const btnTirar= document.getElementById('btn-tirar-foto');
  const contador= document.getElementById('foto-contador');

  galeria.innerHTML = _fotosAtual.map((src, idx) => `
    <div class="foto-thumb">
      <img src="${src}" alt="Foto ${idx + 1}">
      <button type="button" class="foto-thumb-del" onclick="removerFoto(${idx})" title="Remover foto">✕</button>
    </div>
  `).join('');

  wrap.style.display = _fotosAtual.length ? 'block' : 'none';

  const atingiuMax = _fotosAtual.length >= FOTO_MAX;
  btnTirar.disabled = atingiuMax;
  btnTirar.textContent = atingiuMax
    ? `Máximo de ${FOTO_MAX} fotos atingido`
    : (_fotosAtual.length ? '📷 Tirar Outra Foto' : '📷 Tirar Foto');

  contador.textContent = _fotosAtual.length ? `${_fotosAtual.length} de ${FOTO_MAX} foto(s) anexada(s)` : '';
}

function removerFoto(idx) {
  _fotosAtual.splice(idx, 1);
  renderGaleriaFoto();
}

function acionarCamera() {
  if (_fotosAtual.length >= FOTO_MAX) return;
...
document.getElementById('foto-input').addEventListener('change', function(e) {
  ...
  e.target.value = ''; // permite selecionar/tirar a mesma foto de novo em seguida
  if (!file || _fotosAtual.length >= FOTO_MAX) return;
  comprimirImagem(file, function(dataUrl) {
    _fotosAtual.push(dataUrl);
    renderGaleriaFoto();
  });
});

function concluirFotos() {
  if (_fotoItemAtual === null) return fecharPopupFoto();
  if (_fotosAtual.length) {
    nokFotos[_fotoItemAtual] = _fotosAtual.slice();
  } else {
    delete nokFotos[_fotoItemAtual];
  }
  atualizarBadgeFoto(_fotoItemAtual);
  fecharPopupFoto();
}

function fecharPopupFoto() {
  document.getElementById('popup-foto').classList.remove('aberto');
  _fotoItemAtual = null;
  _fotosAtual = [];
}

function atualizarBadgeFoto(n) {
  const b = document.getElementById(`i${n}_fotobadge`);
  if (!b) return;
  const nok = !!document.getElementById(`i${n}_nok`)?.checked;
  const qtd = (nokFotos[n] || []).length;
  b.style.display = nok ? 'inline-flex' : 'none';
  b.classList.toggle('tem-foto', qtd > 0);
  b.textContent = qtd > 0 ? `📷 ${qtd}` : '📷';
}

/* ══════════════════════════════════════════════════════
   AO MARCAR CHECKBOX (individual)
   ══════════════════════════════════════════════════════ */
...
/* ══════════════════════════════════════════════════════
   MARCAÇÃO EM MASSA POR SEÇÃO (cabeçalho "Tudo OK/NOK/NA")
   ══════════════════════════════════════════════════════ */
function marcarGrupo(grp, status) {
  const itens = (ITENS[grp] || []).filter(([, d]) => !isTaco(d));
  if (!itens.length) return;

  function aplicar(carretaEscolhida) {
    itens.forEach(([n]) => {
      const okEl  = document.getElementById(`i${n}_ok`);
      const nokEl = document.getElementById(`i${n}_nok`);
      const naEl  = document.getElementById(`i${n}_na`);
      if (!okEl || !nokEl || !naEl) return;

      okEl.checked  = status === 'ok';
      nokEl.checked = status === 'nok';
      naEl.checked  = status === 'na';

      const tr = document.querySelector(`tr[data-n="${n}"]`);
      if (tr) tr.classList.toggle('nok', status === 'nok');

      if (status === 'nok') {
        if (carretaEscolhida) nokCarreta[n] = carretaEscolhida;
      } else {
        delete nokCarreta[n];
        delete nokFotos[n];
      }
  ...
  const rt = gc('tipo_rodotrem');
  if (status === 'nok' && rt && isSR(grp)) {
    const label = document.querySelector(`.subtitulo[data-grp="${grp}"] .subtitulo-label`)?.textContent || grp;
    pedirCarreta(`Todos os itens NOK de "${label}" pertencem a qual carreta?`, function(op) {
      if (op) aplicar(op);
      // se cancelar, não marca nada em massa
    });
    return;
  }
  aplicar(null);
...
// Injeta os botões "Tudo OK / NOK / NA" em cada cabeçalho de subseção
function inicializarBulkBotoes() {
  document.querySelectorAll('.subtitulo[data-grp]').forEach(el => {
    const grp = el.dataset.grp;
    const label = el.textContent.trim();
    el.innerHTML = `
      <span class="subtitulo-label">${label}</span>
      <span class="bulk-grupo">
        <button type="button" class="bulk-btn bulk-ok"  onclick="marcarGrupo('${grp}','ok')">Tudo OK</button>
        <button type="button" class="bulk-btn bulk-nok" onclick="marcarGrupo('${grp}','nok')">Tudo NOK</button>
        <button type="button" class="bulk-btn bulk-na"  onclick="marcarGrupo('${grp}','na')">Tudo NA</button>
      </span>`;
  });
}
inicializarBulkBotoes();
...
/* ══════════════════════════════════════════════════════
   NOME DE ARQUIVO (PDF/CSV) — cavalo + carreta(s) + motorista
   ══════════════════════════════════════════════════════ */
// nomes (motorista): agrupa qualquer sequ\u00eancia de n\u00e3o-alfanum\u00e9rico num \u00fanico "_"
...
// placas: remove pontua\u00e7\u00e3o (o h\u00edfen do fmtPlaca) sem inserir separador,
// pra n\u00e3o virar "ABC_1234" no nome do arquivo
function sanitizarPlaca(str) {
  return (str || '')
    .toUpperCase()
    .replace(/[^A-Z0-9]/g, '')
    .slice(0, 10);
...
function nomeArquivoBase(osNum) {
  const cavalo    = sanitizarPlaca(gv('placa_cavalo')) || 'SCAVALO';
  const sr1       = sanitizarPlaca(gv('placa_sr1'));
  const sr2       = sanitizarPlaca(gv('placa_sr2'));
  const carretas  = [sr1, sr2].filter(Boolean).join('-');
  const motorista = sanitizarNomeArquivo(gv('motorista')) || 'SMOTORISTA';
  const dataStr   = gv('data') ? gv('data').replace(/-/g, '') : 'SDATA';

  const partes = ['Checklist', cavalo];
  if (carretas) partes.push(carretas);
  partes.push(motorista, dataStr);
  if (osNum) partes.push(osNum);
...
function gerarPDF() {
  ...
  /* ────────────────────────────────────────────────
     REGISTRO FOTOGRÁFICO (fotos anexadas aos itens NOK)
     ──────────────────────────────────────────────── */
  function fotosSecao() {
    // achata { n: [fotos] } numa lista de registros individuais (1 por foto)
    const registros = [];
    Object.keys(nokFotos).map(Number).sort((a,b)=>a-b).forEach(n => {
      const fotos = nokFotos[n] || [];
      fotos.forEach((src, i) => registros.push({ n, src, idx: i+1, total: fotos.length }));
    });
    ...
    function cabecalhoFotos() {
      doc.setFillColor(255,255,255); doc.rect(0,0,W,15,'F');
      doc.setDrawColor(200); doc.setLineWidth(.3); doc.line(0,15,W,15);
      doc.setFontSize(11); doc.setFont('helvetica','bold'); doc.setTextColor(20,20,20);
      doc.text('REGISTRO FOTOGRAFICO - ITENS NOK', ML+2, 9.5);
      try {
        const lg = document.getElementById('logo-pdf');
        if (lg && lg.complete) doc.addImage(lg,'PNG',W-ML-30,0.5,30,14);
      } catch(_) {}
    ...
    const cols    = 2;
    const gap     = 6;
    const boxW    = (cW - gap) / cols;
    const boxImgH = 55;
    const boxH    = boxImgH + 11;
    let col = 0;
    ...
    registros.forEach(r => {
      ...
      doc.rect(x, yF, boxW, boxH);
      try {
        doc.addImage(r.src, 'JPEG', x+2, yF+2, boxW-4, boxImgH);
      } catch(_) {}

      doc.setFontSize(6.5); doc.setFont('helvetica','bold'); doc.setTextColor(20,20,20);
      const rotulo  = r.total > 1 ? `${r.n}. ${desc(d)} (foto ${r.idx}/${r.total})` : `${r.n}. ${desc(d)}`;
      const legenda = doc.splitTextToSize(rotulo, boxW-4);
      doc.text(legenda.slice(0,2), x+2, yF+boxImgH+5.5);

      if (col === cols-1) { col = 0; yF += boxH + gap; } else { col = 1; }
    });
  }
  ...
  // nome do arquivo já sai identificado: Checklist_CAVALO_CARRETA(S)_MOTORISTA_DATA[_OS]
  const baseNome = nomeArquivoBase(osNum);
  ...
  gerarCSV(baseNome, osNum);
}

/* ══════════════════════════════════════════════════════
   GERAÇÃO DO CSV
   ══════════════════════════════════════════════════════ */
function gerarCSV(baseNome, osNum) {
  const tipos=[];
  ...
  const itensFoto = Object.keys(nokFotos).map(Number).sort((a,b)=>a-b)
    .map(n => (nokFotos[n]||[]).length > 1 ? `${n}(${nokFotos[n].length})` : `${n}`);
  ...
  const campos=[
    ...
    obs||'—',
    osNum||'—',
    itensFoto.length ? itensFoto.join('+') : '—'
  ];

  const linha=campos.map(c=>`"${String(c).replace(/"/g,'""')}"`).join(',');
  const cab='"Data","Placa Cavalo","Placa SR1","Placa SR2","Tipo","KM","Motorista","Conferente","Observações","Nº OS","Itens com Foto"';
  const csv='\uFEFF'+cab+'\n'+linha;

  const csvBlob=new Blob([csv],{type:'text/csv;charset=utf-8;'});
  const csvUrl =URL.createObjectURL(csvBlob);
  const link   =document.createElement('a');
  link.href=csvUrl; link.download=`${baseNome}.csv`;
